...
def realtime_emotion_detection():
    ...
    print("Real-time emotion detection started. Speak into the microphone...")

    while True:
        frames = []
        for _ in range(0, int(RATE / CHUNK * 5)):  # Capture 5-second chunks
        ...
        # Skip processing if the audio is silent
        if np.abs(audio_data).mean() < SILENCE_THRESHOLD:
            continue

        # Save the audio chunk to a temporary file
        temp_file = os.path.join(UPLOAD_FOLDER, "temp_realtime.wav")
        ...
        result = whisper_model.transcribe(temp_file, verbose=False)
...
# Start real-time emotion detection in a separate thread
realtime_thread = threading.Thread(target=realtime_emotion_detection)
